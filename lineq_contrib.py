...
keys = data.columns
A0 = data[[key for key in keys if key.startswith('D')]].values
A0 = np.array([a / a.mean() for a in A0])

B0 = data[[key for key in keys if key.startswith('LS')]].values

if __name__ == '__main__':

    W, s, Wh = LA.svd(B0.T @ B0)

    A, A_test, B, B_test = train_test_split(A0, B0, test_size=0.2)
    ...
    ax.plot(ps, np.array(Es1)/error(B1), ps, np.array(Es2)/error(B1_test))
